...
import { ScrimFormData, Scrim } from '@/types'
...
export function ScrimForm({
  ...
  const dayOptions = APP_CONFIG.daysOfWeek.map(d => ({
    value: d,
    label: formatDayLong(d),
  }))

  const hourOptions = APP_CONFIG.availableHours.map(h => ({
    value: h,
    label: formatHour(h),
  }))
